...
class BlockStatus(BaseModel):
    ...
    BlockStatus
    """
    accounts: Optional[Dict[str, BlockStatusAccountsValue]] = None
    hosts: Optional[Dict[str, BlockStatusHostsValue]] = None
    __properties = ["accounts", "hosts"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> BlockStatus:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each value in hosts (dict)
        _field_dict = {}
        if self.hosts:
            for _key in self.hosts:
                if self.hosts[_key]:
                    _field_dict[_key] = self.hosts[_key].to_dict()
            _dict['hosts'] = _field_dict
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> BlockStatus:
        """Create an instance of BlockStatus from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return BlockStatus.parse_obj(obj)

        _obj = BlockStatus.parse_obj({
            "accounts": dict(
                (_k, BlockStatusAccountsValue.from_dict(_v))
                for _k, _v in obj.get("accounts").items()
            )
            if obj.get("accounts") is not None
            else None,
            "hosts": dict(
                (_k, BlockStatusHostsValue.from_dict(_v))
                for _k, _v in obj.get("hosts").items()
            )
