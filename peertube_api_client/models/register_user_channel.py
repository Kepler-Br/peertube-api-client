...
class RegisterUserChannel(BaseModel):
    """
    channel base information used to create the first channel of the user
    """
    name: Optional[constr(strict=True, max_length=50, min_length=1)] = Field(None, description="immutable name of the channel, used to interact with its actor")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RegisterUserChannel:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> RegisterUserChannel:
        """Create an instance of RegisterUserChannel from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RegisterUserChannel.parse_obj(obj)

        _obj = RegisterUserChannel.parse_obj({
            "name": obj.get("name"),
