...
class GetMeVideoRating(BaseModel):
    """
    GetMeVideoRating
    """
    id: conint(strict=True, ge=1) = Field(...)
    rating: StrictStr = Field(..., description="Rating of the video")
    __properties = ["id", "rating"]

    @validator('rating')
    def rating_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('like', 'dislike', 'none'):
            raise ValueError("must be one of enum values ('like', 'dislike', 'none')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetMeVideoRating:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GetMeVideoRating:
        """Create an instance of GetMeVideoRating from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetMeVideoRating.parse_obj(obj)

        _obj = GetMeVideoRating.parse_obj({
            "id": obj.get("id"),
