...
class NotificationVideoAbuse(BaseModel):
    """
    NotificationVideoAbuse
    """
    id: Optional[conint(strict=True, ge=1)] = None
    video: Optional[NotificationVideoAbuseVideo] = None
    __properties = ["id", "video"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NotificationVideoAbuse:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> NotificationVideoAbuse:
        """Create an instance of NotificationVideoAbuse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NotificationVideoAbuse.parse_obj(obj)

        _obj = NotificationVideoAbuse.parse_obj({
            "id": obj.get("id"),
            "video": NotificationVideoAbuseVideo.from_dict(obj.get("video")) if obj.get("video") is not None else None
        })
        return _obj
