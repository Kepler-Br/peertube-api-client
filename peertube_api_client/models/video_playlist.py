...
class VideoPlaylist(BaseModel):
    ...
    display_name: Optional[constr(strict=True, max_length=120, min_length=1)] = Field(None, alias="displayName")
    is_local: Optional[StrictBool] = Field(None, alias="isLocal")
    video_length: Optional[conint(strict=True, ge=0)] = Field(None, alias="videoLength")
    thumbnail_path: Optional[StrictStr] = Field(None, alias="thumbnailPath")
    privacy: Optional[VideoPlaylistPrivacyConstant] = None
    type: Optional[VideoPlaylistTypeConstant] = None
    owner_account: Optional[AccountSummary] = Field(None, alias="ownerAccount")
    video_channel: Optional[VideoChannelSummary] = Field(None, alias="videoChannel")
    __properties = ["id", "uuid", "shortUUID", "createdAt", "updatedAt", "description", "displayName", "isLocal", "videoLength", "thumbnailPath", "privacy", "type", "ownerAccount", "videoChannel"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> VideoPlaylist:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of video_channel
        if self.video_channel:
            _dict['videoChannel'] = self.video_channel.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> VideoPlaylist:
        """Create an instance of VideoPlaylist from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return VideoPlaylist.parse_obj(obj)

        _obj = VideoPlaylist.parse_obj({
            "id": obj.get("id"),
            "uuid": obj.get("uuid"),
            ...
            "video_length": obj.get("videoLength"),
            "thumbnail_path": obj.get("thumbnailPath"),
            "privacy": VideoPlaylistPrivacyConstant.from_dict(obj.get("privacy")) if obj.get("privacy") is not None else None,
            "type": VideoPlaylistTypeConstant.from_dict(obj.get("type")) if obj.get("type") is not None else None,
            "owner_account": AccountSummary.from_dict(obj.get("ownerAccount")) if obj.get("ownerAccount") is not None else None,
            "video_channel": VideoChannelSummary.from_dict(obj.get("videoChannel")) if obj.get("videoChannel") is not None else None
        })
        return _obj
